...
<template>
  <div class="sticky-nav-bar">
    <van-nav-bar :safe-area-inset-top="true" class="nav-bar-box" @click-left="onClickLeft" @click-right="onClickRight">
      <!-- 标题内容 -->
      <template #title>
        <span class="nav-title-text-box">{{ navBarTitle }}</span>
      </template>

      <!-- 左侧内容 -->
      <template #left>
        <img src="../assets/back.png" alt="" class="back-icon-box" v-if="isShowLeftArrow" />
        <span class="nav-leftOrRight-text-box">{{ navBarLeftText }}</span>
      </template>

      <!-- 右侧内容 -->
      <template #right>
        <span class="nav-leftOrRight-text-box">{{ navBarRightText }}</span>
      </template>
    </van-nav-bar>
  </div>
</template>
